...
import ir.part.sdk.ara.util.androidsvg.SVGParser.TextScanner;
   ...
   */
    ...
    private Ruleset parseRuleset(CSSTextScanner scan) {
        ...
        try {
            ...
            Timber.e("CSS parser terminated early due to error: %s", e.getMessage());
            if (LibConfig.DEBUG)
    ...
    static class Ruleset {
        ...
        void removeFromSource() {
            ...
            while (iter.hasNext()) {
                if (iter.next().source == Source.RenderOptions)
                    iter.remove();
            }
        }
...
//         simpleSelectors.addedAttributeOrPseudo();
    ...
    private static class PseudoClassAnPlusB implements PseudoClass {
        ...
        public String toString() {
            ...
            return isOfType ? String.format(Locale.ENGLISH, "nth-%schild(%dn%+d of type <%s>)", last, a, b, nodeName)
                    : String.format(Locale.ENGLISH, "nth-%schild(%dn%+d)", last, a, b);
        }
